<template>
    <section class="bigCard">
      ...
      <section class="baseStat">
        ...
        <input @input="saveForm" type="text" id="charName" v-model="character.name">
      </section>
      <section class="baseStat">
    ...
    <section class="colHolder">
        ...
        <section class="rightCol">
            <button :class="{'conditionButtonActive': career.barbarian, 'conditionButton': !career.barbarian}" @click="toggleClass('barbarian')">Barbarian</button>
            <section v-if="career.barbarian" >
                <h3>Rage</h3>
                <section class="row">
                    <input type="number" v-model.number="careerValues.barbarian.rage"></input>
                    <section class="plusMinusButtons">
                        <button class="plusMinusButton" @click="incrementResource('barbarian', 'rage')">+</button>
                        <button class="plusMinusButton" @click="decrementResource('barbarian', 'rage')">-</button>
                    </section>
                </section>
                <section class="cardCareer">
                    <h4>Effects</h4>
                    <ul>
                        <li>+{{ careerValues.barbarian.rage }}d4 damage on attacks.</li>
                        <li>You are Off-Guard</li>
                    </ul>

                    <h4>To End Your Rage</h4><br>
                    Flat Check, DC {{ 10 + careerValues.barbarian.rage }}<br>
                    <ul>
                        <li><b>Success:</b> Fury ends. Pass out for 1d4 rounds if Fatigued.</li>
                        <li><b>Fail:</b> Fatigue + 1, Fury ends, and if you were already Fatigued you pass out for 1d4 minutes.</li>
                        <li><b>Crit Fail:</b> stay Furious. If you are Fatigued, you are Overcome By Rage.</li>
                    </ul>
                </section>
            </section>
        </section>
    ...
    export default {
        ...
        methods: {
            ...
            toggleClass(className) {
                this.career[className] = !this.career[className];
                this.saveForm();
            },
            incrementResource(career, res) {
                console.log(this);
                this['careerValues'][career][res]++;
                if(res === "rage") {
                    this.character.dieAtDying++;
                }
                this.saveForm();
            },
            decrementResource(career, res) {
                if(this.careerValues[career][res] < 1) return;
                this.careerValues[career][res]--;
                if(res === "rage") {
                    this.character.dieAtDying--;
                }
            ...
            saveForm() {
                ...
                let career = JSON.stringify(this.career);
                let careerValues = JSON.stringify(this.careerValues);
                localStorage.setItem('character', character);
                localStorage.setItem('conditions', conditions);
                console.log(this.career.barbarian);
                localStorage.setItem('career', career);
                localStorage.setItem('careerValues', careerValues);
            },
            loadForm() {
                let character = localStorage.getItem('character');
                let conditions = localStorage.getItem('conditions');
                let careers = localStorage.getItem('career');
                let careerValues = localStorage.getItem('careerValues');
                let char = JSON.parse(character);
                let cond = JSON.parse(conditions);
                let car = JSON.parse(careers);
                let carV = JSON.parse(careerValues);
                if(!char || !cond || !car || !carV) {
                    return;
                }
                if(char) this.character = char;
                if(cond) this.conditions = cond;
                if(car) this.career = car;
                if(carV) this.careerValues = carV;
            },
            resetCharacter() {
                let conditions = Object.keys(this.conditions);
                let charStats = Object.keys(this.character);
                let careers = Object.keys(this.career);
                ...
                careers.forEach((car) => {
                    this.career[car] = false;
                    let resources = Object.keys(this.careerValues[car]);
                    resources.forEach((res) => {
                        this.careerValues[car][res] = 0;
                    })
                })
                this.character.name = "";
